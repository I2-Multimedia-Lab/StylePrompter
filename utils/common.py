...
def vis_images(log_hooks, score_log=None):
	display_count = len(log_hooks)
	fig = plt.figure(figsize=(3 * len(log_hooks[0]), 4 * display_count))
	gs = fig.add_gridspec(display_count, len(log_hooks[0]))
	for i in range(display_count):
		hooks_dict = log_hooks[i]
		if score_log is not None:
			score_dict = score_log[i]
			for idx, keys in enumerate(hooks_dict):
				fig.add_subplot(gs[i, idx])
				plt.imshow(hooks_dict[keys])
				plt.title('{}\nSim={:.2f}'.format(str(keys), float(score_dict[idx])))
		else:
