...
class CoachBaseline:
    def __init__(self, opts):
        ...
        # Initialize network
        self.net = StylePrompterBaseline(self.opts).to(self.device)
    ...
    def train(self):
        ...
        while self.global_step < self.opts.max_steps:
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                y_hat, latents, _ = self.net(x)
                loss, loss_dict, id_logs = self.calc_loss(x, y, y_hat, latents=latents)
    ...
    def calc_loss(self, x, y, y_hat, latents=None):
        loss_dict = {}
        loss = 0.0
        id_logs = None
        ...
        # latent loss
        if self.opts.align_lambda > 0 and latents is not None:
            b, n, _ = latents.shape
            loss_align = F.mse_loss(latents, self.net.latent_avg.repeat(b, n, 1))
